...
APP.onRouteChange = function (path) {
    APP.route.beforeRender(function (data) {
        ...
        if (APP.currentUser) {
            ...
            if (APP.route.guest[path]) {
                APP.route.guest[path](data);
            } else {
                APP.route.guest['/'](data);
            }
        }
    });
};
...
APP.render = function (tpl, data, options) {
    ...
    var html = (typeof tpl === 'function') ? tpl(data) : tpl;

    if (options.layout !== false) {
        html = APP.TEMPLATE[options.layout || 'layout'](data).replace('{{yield}}', html);
    }
